...
function removeRunDependency(id) {
  console.log('removeRunDependency !')
  runDependencies--;
  if (Module["monitorRunDependencies"]) {
    Module["monitorRunDependencies"](runDependencies);
  }
...
var wasmBinaryFile = "https://cdn.jsdelivr.net/gh/visualcamp/seeso-js-cdn/simd-pthread/4/gaze.wasm";
...
function createWasm() {
  ...
  function receiveInstance(instance, module) {
    ...
    wasmModule = module;
    if (!ENVIRONMENT_IS_PTHREAD) {
      console.log('EVMENT IS NOT PTHREAD !')
...
var ASM_CONSTS = {
  ...
  1315: function() {
    var jsString = window.getCookie("uuid");
    var lengthBytes = lengthBytesUTF8(jsString) + 1;
    var stringOnWasmHeap = _malloc(lengthBytes);
    stringToUTF8(jsString, stringOnWasmHeap, lengthBytes);
  ...
  1557: function() {
    var jsString = window.navigator.userDeviceType;
    var lengthBytes = lengthBytesUTF8(jsString) + 1;
    var stringOnWasmHeap = _malloc(lengthBytes);
    stringToUTF8(jsString, stringOnWasmHeap, lengthBytes);
    window.navigator.userDeviceType = null;
    return stringOnWasmHeap;
  },
  1821: function() {
    var jsString = window.navigator.browser;
    var lengthBytes = lengthBytesUTF8(jsString) + 1;
    var stringOnWasmHeap = _malloc(lengthBytes);
    stringToUTF8(jsString, stringOnWasmHeap, lengthBytes);
    window.navigator.browser = null;
    return stringOnWasmHeap;
  },
  2140: function($0, $1) {
    onGaze($0, $1);
  ...
  2163: function($0, $1, $2) {
    onCalibrationProcess($0, $1, $2);
  },
  2205: function($0, $1) {
    onCalibrationFinished($0, $1);
  },
  2869818: function() {
    var statusCodeJsString = window.statusCode + "";
    var lengthBytes = lengthBytesUTF8(statusCodeJsString) + 1;
    var stringOnWasmHeap = _malloc(lengthBytes);
    stringToUTF8(statusCodeJsString, stringOnWasmHeap, lengthBytes);
    return stringOnWasmHeap;
  },
  2870102: function() {
    var seesoCodeJsString = window.seesoCode + "";
    var lengthBytes = lengthBytesUTF8(seesoCodeJsString) + 1;
    var stringOnWasmHeap = _malloc(lengthBytes);
    stringToUTF8(seesoCodeJsString, stringOnWasmHeap, lengthBytes);
    return stringOnWasmHeap;
  },
  2870347: function() {
    var sidJsString = window.sid + "";
    var lengthBytes = lengthBytesUTF8(sidJsString) + 1;
    var stringOnWasmHeap = _malloc(lengthBytes);
    stringToUTF8(sidJsString, stringOnWasmHeap, lengthBytes);
    return stringOnWasmHeap;
  },
  2870564: function() {
    return Object.keys(window.headers).length;
  },
  2870611: function($0) {
    var index = $0;
    var key = Object.keys(window.headers)[index].toString();
    var value = window.headers[key];
  ...
  2870885: function($0) {
    ...
    var value = window.headers[key].toString();
  ...
  3177411: function($0, $1) {
    setTimeout(function() {
      __emscripten_do_dispatch_to_thread($0, $1);
    }, 0);
  },
};
function do_fetch(urlCppString, cookieCppString) {
  return Asyncify.handleSleep((wakeUp) => {
    const url = UTF8ToString(urlCppString);
    const cookie = UTF8ToString(cookieCppString);
    fetch(url, {
      method: "GET",
      headers: { "cookie-force": cookie, "user-connection-type": "web" },
    }).then((res) => {
      window.statusCode = res.status || "undefined";
      window.seesoCode = res.headers.get("seeso-code") || "undefined";
      window.sid = res.headers.get("sid") || "undefined";
      var headers = {};
      ...
      res.text().then(function(resData) {
        var lengthBytes = lengthBytesUTF8(resData) + 1;
        var stringOnWasmHeap = _malloc(lengthBytes);
        stringToUTF8(resData, stringOnWasmHeap, lengthBytes);
        wakeUp(stringOnWasmHeap);
      });
    });
  });
}
...
var FS = {
  ...
  createPreloadedFile: function(
    ...
    console.log('FS . createPreloadedFile !')
    Browser.init();
...
function ___sys_getdents64(fd, dirp, count) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(4, 1, fd, dirp, count);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    if (!stream.getdents) {
...
function _emscripten_proxy_to_main_thread_js(index, sync) {
  ...
  if (numCallArgs > 20 - 1)
    throw "emscripten_proxy_to_main_thread_js: Too many arguments " +
      numCallArgs +
...
function _environ_get(__environ, environ_buf) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(10, 1, __environ, environ_buf);
  try {
    var bufSize = 0;
...
function _fd_close(fd) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(12, 1, fd);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    FS.close(stream);
    return 0;
...
function _fd_fdstat_get(fd, pbuf) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(13, 1, fd, pbuf);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    var type = stream.tty
      ? 2
...
function _sysconf(name) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(17, 1, name);
  switch (name) {
    case 30:
      return 16384;
    case 85:
  ...
  handleSleep: function(startAsync) {
    ...
    if (Asyncify.state === Asyncify.State.Normal) {
      ...
      startAsync(function(handleSleepReturnValue) {
        assert(
          !handleSleepReturnValue || typeof handleSleepReturnValue === "number"
        );
        if (ABORT) return;
...
var _initEyeTracker = (Module["_initEyeTracker"] = createExportWrapper(
  "initEyeTracker"
));
var _free = (Module["_free"] = createExportWrapper("free"));
var _deinitEyeTracker = (Module["_deinitEyeTracker"] = createExportWrapper(
  "deinitEyeTracker"
));
var _setCalibrationData = (Module["_setCalibrationData"] = createExportWrapper(
  "setCalibrationData"
));
var _setCalibrationRegion = (Module[
  "_setCalibrationRegion"
] = createExportWrapper("setCalibrationRegion"));
var _startCalibration = (Module["_startCalibration"] = createExportWrapper(
  "startCalibration"
));
var _startCollectSamples = (Module[
  "_startCollectSamples"
] = createExportWrapper("startCollectSamples"));
var _stopCalibration = (Module["_stopCalibration"] = createExportWrapper(
  "stopCalibration"
));
var _setCameraHorizontalFOV = (Module[
  "_setCameraHorizontalFOV"
] = createExportWrapper("setCameraHorizontalFOV"));
var _addFrame = (Module["_addFrame"] = createExportWrapper("addFrame"));
var _registerCallback = (Module["_registerCallback"] = createExportWrapper(
  "registerCallback"
));
var _create_buffer = (Module["_create_buffer"] = createExportWrapper(
  "create_buffer"
));
var _malloc = (Module["_malloc"] = createExportWrapper("malloc"));
var _destroy_buffer = (Module["_destroy_buffer"] = createExportWrapper(
  "destroy_buffer"
));
...
var _emscripten_proxy_main = (Module[
...
var __emscripten_thread_init = (Module[
...
var dynCall_vijffffi = (Module["dynCall_vijffffi"] = createExportWrapper(
  "dynCall_vijffffi"
));
var dynCall_vifff = (Module["dynCall_vifff"] = createExportWrapper(
  "dynCall_vifff"
));
...
var dynCall_viiiiiiiii = (Module["dynCall_viiiiiiiii"] = createExportWrapper(
  "dynCall_viiiiiiiii"
));
...
var dynCall_viiiiiiiiii = (Module["dynCall_viiiiiiiiii"] = createExportWrapper(
  "dynCall_viiiiiiiiii"
));
var dynCall_viiiii = (Module["dynCall_viiiii"] = createExportWrapper(
  "dynCall_viiiii"
));
var dynCall_viiiiiiiddi = (Module["dynCall_viiiiiiiddi"] = createExportWrapper(
  "dynCall_viiiiiiiddi"
));
var dynCall_viiiiiiiiiiddi = (Module[
  "dynCall_viiiiiiiiiiddi"
] = createExportWrapper("dynCall_viiiiiiiiiiddi"));
var dynCall_viiid = (Module["dynCall_viiid"] = createExportWrapper(
  "dynCall_viiid"
));
var dynCall_diii = (Module["dynCall_diii"] = createExportWrapper(
  "dynCall_diii"
));
var dynCall_viiiid = (Module["dynCall_viiiid"] = createExportWrapper(
  "dynCall_viiiid"
));
var dynCall_viidi = (Module["dynCall_viidi"] = createExportWrapper(
  "dynCall_viidi"
));
var dynCall_vidii = (Module["dynCall_vidii"] = createExportWrapper(
  "dynCall_vidii"
));
...
var dynCall_viiiiiii = (Module["dynCall_viiiiiii"] = createExportWrapper(
  "dynCall_viiiiiii"
));
var dynCall_viid = (Module["dynCall_viid"] = createExportWrapper(
  "dynCall_viid"
));
var dynCall_v = (Module["dynCall_v"] = createExportWrapper("dynCall_v"));
var dynCall_viiiiiiiiidd = (Module[
  "dynCall_viiiiiiiiidd"
] = createExportWrapper("dynCall_viiiiiiiiidd"));
...
] = createExportWrapper("dynCall_iiiiiifiididiii"));
...
var dynCall_viijj = (Module["dynCall_viijj"] = createExportWrapper(
  "dynCall_viijj"
));
var dynCall_viiijjjj = (Module["dynCall_viiijjjj"] = createExportWrapper(
  "dynCall_viiijjjj"
));
var dynCall_viiiiiiiiiiii = (Module[
  "dynCall_viiiiiiiiiiii"
] = createExportWrapper("dynCall_viiiiiiiiiiii"));
var dynCall_viiiiiiiiiii = (Module[
  "dynCall_viiiiiiiiiii"
] = createExportWrapper("dynCall_viiiiiiiiiii"));
var dynCall_viiiif = (Module["dynCall_viiiif"] = createExportWrapper(
  "dynCall_viiiif"
));
...
var dynCall_vjii = (Module["dynCall_vjii"] = createExportWrapper(
  "dynCall_vjii"
));
...
if (!Object.getOwnPropertyDescriptor(Module, "Asyncify"))
  Module["Asyncify"] = function() {
    abort(
      "'Asyncify' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
    );
  };
if (!Object.getOwnPropertyDescriptor(Module, "Fibers"))
  Module["Fibers"] = function() {
    abort(
      "'Fibers' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
    );
  };
...
if (!Object.getOwnPropertyDescriptor(Module, "WS"))
  Module["WS"] = function() {
    abort(
      "'WS' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
    );
  };
...
dependenciesFulfilled = function runCaller() {
  if (!calledRun) {
    console.log('dependenciesFulFilled = run()');
    run();
  }
